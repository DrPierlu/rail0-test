...
# and are covered by the SDK end-to-end flows in rail0-test/go/.

INDEXER_TX_HASH  = ("0x" + "dc" * 32).freeze
INDEXER_CHAIN_ID = 1   # placeholder — no transaction with this hash exists in the DB
...
describe "PUT /sync/chains/:chain_id/transactions/:tx_hash — authentication" do
  ...
  it "returns 401 with a tampered signature" do
    ...
    http_request(:Put, "/sync/chains/#{INDEXER_CHAIN_ID}/transactions/#{INDEXER_TX_HASH}",
                 body: body_str,
                 headers: {
...
describe "PUT /sync/chains/:chain_id/transactions/:tx_hash — validation" do
  ...
  it "returns 422 for an unknown event_type on confirm" do
    put_sync(INDEXER_CHAIN_ID, INDEXER_TX_HASH,
  ...
  # in the URL does not match the transaction's blockchain. Reaching this
  # validation requires a real transaction in submitted state, which only exists
  # after the full broadcast flow. That case is covered by the gateway unit test
  # `test_put__sync_confirm_422_for_chain_mismatch` in sync_test.rb.
end
